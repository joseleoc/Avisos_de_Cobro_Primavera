...
const DEFAULT_EMISSION_DATE = "01/01/22";
const DEFAULT_EXPIRATION_DATE = "31/12/22";
...
export const cliInputs = async (): Promise<CLIInputs> => {
  try {
    ...
    // Obtain Dolar price
    const price = Number(
      await question("Ingrese el precio de dolar en bolivares: ", DEFAULT_DOLLAR_PRICE.toString())
    );
    if (isNaN(price)) {
    ...
    const emissionDate = await question(
      "Ingrese la fecha de emisión en formato dd/mm/aa: ", DEFAULT_EMISSION_DATE
    );
    ...
    const expirationDate = await question(
      "Ingrese la fecha de expiración en formato dd/mm/aa: ", DEFAULT_EXPIRATION_DATE
    );
